...
final class LLMWordClient {
    ...
    private var systemPrompt: String {
        ...
        A single capitalized Latin-script word must be tested as a German noun before it is interpreted as English. If it is a valid German noun, preserve that exact headword even when the spelling is also an English word. For example, input "Route" must return the noun "Route" with gender "die"; never redirect it to "Wegbeschreibung". If the user includes an article, such as "die Route", return the headword without the article and use the article to identify the noun entry.
